...
namespace pylorak.TinyWall
{
    internal partial class ConnectionsForm : Form
    {
        ...
        internal ConnectionsForm(TinyWallController ctrl)
        {
            ...
            this.RefreshTimer.Tick += (sender, args) => NetworkActivityRefresh.TryRun(UpdateList);
        }
        ...
        private void UpdateList()
        {
            ...
            foreach (TcpRow tcpRow in tcpTable)
            {
                  ...
                  || (chkShowActive.Checked && (tcpRow.State != TcpState.Listen)))
                {
                    ...
                    string state = tcpRow.State == TcpState.Listen
                        ? NetworkActivityStatusClassifier.ToDisplayText(NetworkActivityStatus.Listening)
                        : tcpRow.State.ToString();
                    ConstructListItem(itemColl, pi, "TCP", tcpRow.LocalEndPoint, tcpRow.RemoteEndPoint, state, now, RuleDirection.Invalid);
                }
            }
            ...
            if (chkShowListen.Checked)
            {
                ...
                foreach (UdpRow udpRow in udpTable)
                {
                    ...
                    ConstructListItem(itemColl, pi, "UDP", udpRow.LocalEndPoint, dummyEP,
                        NetworkActivityStatusClassifier.ToDisplayText(NetworkActivityStatus.Listening),
                        now, RuleDirection.Invalid);
                }
            ...
            if (chkShowBlocked.Checked || chkShowActive.Checked)
            {
                ...
                for (int i = 0; i < fwLog.Length; ++i)
                {
                        ...
                        case EventLogEvent.ALLOWED:
                            {
                                ...
                                status = NetworkActivityStatus.Allowed;
                                break;
                            }
                        ...
                        case EventLogEvent.BLOCKED:
                            {
                                ...
                                status = NetworkActivityStatus.Blocked;
                                break;
                            }
                        default:
                            continue;
                    ...
                    if (!NetworkActivityStatusClassifier.IsDecisionVisible(
                        status,
                ...
                for (int i = 0; i < filteredLog.Count; ++i)
                {
                    ...
                    NetworkActivityStatus status = NetworkActivityStatusClassifier.FromFirewallDecision(
                        entry.Event == EventLogEvent.ALLOWED);
